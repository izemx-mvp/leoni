...
export const DOSSIER_DEMO: DossierOnboarding = {
  candidatId: "CAN-2026-01261",
  arrivee: {
    date: "03/08/2026",
    heure: "08:00",
    site: "Bouskoura",
    pointAccueil: POINTS_ACCUEIL[0],
    contactRH: "Amina Rajouh",
  ...
  documents: documentsPreselectionnes("2026-08-01").map((d, i) => ({
    ...
    demandeLe: "27/07/2026",
    recuLe: i < 5 ? "28/07/2026" : undefined,
    valideLe: i < 3 ? "28/07/2026" : undefined,
    validePar: i < 3 ? "Amina Rajouh" : undefined,
  })),
  equipements: equipementsPourPoste("Opératrice câblage").map((e, i) => ({
    ...e,
    taille: e.id === "EPI-CHAUSSURES" ? "38" : e.id === "EPI-BADGE" ? "" : "M",
    statut: i === 2 ? "À commander" : "Disponible",
  })),
  tailles: { blouse: "M", gilet: "M", chaussures: "38", gants: "M" },
  vestiaire: {
    statut: "Affecté",
    vestiaire: "Vestiaire femmes B",
    casier: "214",
    cle: "CLE-214",
    dateRemise: "",
    checklist: checklistDepuis(["Casier attribué", "Clé remise", "Casier communiqué au salarié"], ["Casier attribué"]),
  },
  transport: {
    statut: "Confirmé",
    besoin: true,
  ...
  preparation: checklistDepuis(CHECKLIST_PREPARATION, [
    "Matricule créé",
    "Badge préparé",
    "Casier affecté",
    "Gilet préparé",
  ...
  communications: [
    {
      ...
      heure: "16:42",
      canal: "WhatsApp",
      objet: "Confirmation de votre candidature et informations d'intégration",
      contenu: "Message d'intégration envoyé avec les documents à fournir, le point d'accueil et le trajet de transport.",
      statut: "Lu",
      etapes: ["Message généré", "Message validé RH", "Envoyé", "Lu"],
    },
    {
      id: "COM-DEMO-2",
      date: "29/07/2026",
      heure: "09:10",
      canal: "Email",
      objet: "Rappel — documents à fournir avant votre intégration",
      contenu: "Relance sur les documents restants : certificat médical d'aptitude et attestation de travail.",
      statut: "Envoyé",
      etapes: ["Message généré", "Envoyé"],
    },
  ],
...
export function dossierPourOuvrier(o: {
  ...
  return {
    ...
    documents: documentsPreselectionnes(o.dateIntegration).map((d, i) => ({
      ...
      validePar: integre || i < 3 ? "Amina Rajouh" : undefined,
    })),
